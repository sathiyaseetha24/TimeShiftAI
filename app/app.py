...
st.markdown("""
    <style>
        /* Hide sidebar and unwanted arrow text */
...
# --- Row 1: Financial Inputs ---
colA, colB, colC = st.columns(3)
with colA:
    currency = st.selectbox("Preferred Currency 💱", ["AED", "USD", "INR", "EUR", "GBP"])
    salary = st.number_input(f"Current Monthly Salary ({currency})", value=15000, step=500)
with colB:
    family_expense = st.number_input(f"Monthly Family Expense ({currency})", value=5000, step=500)
    emi = st.number_input(f"Total Monthly EMIs ({currency})", value=2000, step=500)
with colC:
